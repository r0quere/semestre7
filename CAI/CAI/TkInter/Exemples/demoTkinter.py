...
class ToolBarDemo(Demo):
	label='ToolBar'
	def __init__(self,master):
		Demo.__init__(self,master)
		l = Label(self, text="button 1", relief="flat", bd=4)
...
class TextDemo(Demo):
	...
	Highlights = {'#.*': Options(foreground='red'),
				  ...
				  r'\bclass\b\s.*\n':Options(background='pink', spacing1=15),
	...
	def __init__(self, master):
		...
		if sys.argv[0] == '':
			file_name = "/mnt/d/Mes Documents/Python/tkinter/tkDemo.py"
		else:
...
class Application:
	...
	def Go(self):
		MenubarDemo(root)
		self.toolBarDemo=ToolBarDemo(root)
