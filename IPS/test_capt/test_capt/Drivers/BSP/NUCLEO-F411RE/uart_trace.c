...
int uart_vprintf(const char *msg, va_list ap){
   ...
   // status = HAL_UART_Transmit_DMA(&huart2, (uint8_t*)uart_buffer, n );
    status = HAL_UART_Transmit(&huart2, (uint8_t*)uart_buffer, n, 0xFFFF);
    //char *msg55 = "He555llo Nucleo Fun!\n\r";
    //status = HAL_UART_Transmit(&huart2, (uint8_t*)msg55, strlen(msg55), 0xFFFF);
